          ...
          builder: (context, child) {
            return Consumer<DatabaseEvent?>(
              builder: (context, event, child) {
                ...
                var appointmentid = queues.where(
                    (elemenent) => elemenent.appointmentId == widget.user.uid);

                //log(appointmentid.toString());

                int getIndex() {
                  int index = queues.indexWhere(
                      ((element) => element.appointmentId == widget.user.uid));
                  return index;
                }
                ...
                return FutureBuilder(
                    future: DatabaseController.withoutUID()
                        .getSingleAppointment('K5xtiqwfDascAWS42tBg'),
                    builder: (_, snapshot) {
